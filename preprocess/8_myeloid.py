...
sc.settings.figdir = '/path/to/your/fig/myeloid'
...
adata = sc.read_h5ad(wd + 'myeloid.h5ad')

# we run scrublet to detect doublts
# scrublet uses un-normalized matrix.
#sc.external.pp.scrublet(adata, batch_key='sample', verbose=False)
...
sc.pp.regress_out(adata, ['total_counts', 'total_counts_mt', 'nFeature_RNA', 'nCount_RNA', 'percent_mt', 'S_score', 'G2M_score'])
...
sce.pp.harmony_integrate(adata, key=['sample', 'donor'], adjusted_basis = 'X_pca_harmony')
...
sc.tl.leiden(adata, resolution=0.6, n_iterations=2, key_added='leiden_0_6', random_state=seed)
